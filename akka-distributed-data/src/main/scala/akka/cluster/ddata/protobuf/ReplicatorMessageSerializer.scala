...
private[akka] object ReplicatorMessageSerializer {
   ...
   */
  class SmallCache[A <: AnyRef, B <: AnyRef](size: Int) {
    ...
    /**
     * Get value from cache or create new value and add
     * it to the cache. Duplicates of same value may be added
     * if multiple threads call this concurrently, but decent
     * (low cost) effort is made to reduce the chance of duplicates.
     */
    def getOrAdd(a: A, b: () ⇒ B): B = {
      val position = n.get
...
class ReplicatorMessageSerializer(val system: ExtendedActorSystem)
  ...
  private val readCache = new SmallCache[Read, Array[Byte]](4)
  private val writeCache = new SmallCache[Write, Array[Byte]](4)
  ...
  def toBinary(obj: AnyRef): Array[Byte] = obj match {
    ...
    case m: Write          ⇒ writeCache.getOrAdd(m, () ⇒ writeToProto(m).toByteArray)
    case WriteAck          ⇒ writeAckBytes
    case m: Read           ⇒ readCache.getOrAdd(m, () ⇒ readToProto(m).toByteArray)
